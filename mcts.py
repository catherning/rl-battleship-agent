...
class MCTS:
    ...
    def __init__(self, field: FieldState, nnet, sims, cpuct):
        self.field = field
        self.nnet = nnet
        self.sims = sims
    ...
    def get_action_probabilities(self, ships, damages, t=1):
        ...
        visible_area = self.field.state_matrix
        counts = []
        ...
        # bestA is best action to do, you take the best one
        # Before 15 steps, t=0 so take best. After 15, samples
        if t == 0:
    ...
    def search(self, ships, damages):  # , depth=0, depth_limit=None):
        ...
        if s not in self.Ps:
            ...
            valids_moves = self.field.possible_moves()
            valids = np.zeros(self.field.get_num_actions())
            for coordinates in valids_moves:
                valids[self.field.field_size * coordinates[0] + coordinates[1]]
            ...
            if sum_Ps_s > 0:
                ...
                # or you've get over-fitting or something else.
                # If you have got dozens or hundreds of these messages you should pay attention to your NNet
        ...
        # TODO check if still ok even if remove the pass action ?
        for a in range(self.field.get_num_actions()):
            if valids[a]:
                if (s, a) in self.Qsa:
        ...
        # TODO change
        next_d, _ = self.field.get_next_state(ships, damages, 1, a)
