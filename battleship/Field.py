...
class Field():
    '''
    Logic of the game, how ships should bplaced and how winner determined
    ...
    3. Ships cannot be placed near to the border
    4. There should be distance of >1 between ships
    5. Only ships with sizes more than 1 are available, because it is impossible to gain 
       insights about places of single cell ships, random will perform the same as NN
    ...
    def place_ship(self, ships, new_ship_size):
        ...
        while placed is False:
            ...
            x = ceil(random.random()*self.n)-2-new_ship_size*z          # avoid border and out of battlefield cells
            y = ceil(random.random()*self.n)-2-new_ship_size*(1-z)    # avoid border and out of battlefield cells
...
def print_battlefield(ships):
    '''
    Print out the battlefield, ships or damages or visible
    '''
    ...
    for y in range(n):
        ...
        for x in range(n):
            pos = ships[y][x]
            if pos == -1: print("x ",end="")    # dead ship cell
            elif pos == 1: print("o ",end="")   # alive ship cell
            elif pos == 2: print("v ",end="")   # don't remember what this for
            elif pos == 3: print("s ",end="") # second player hits
            elif pos == 5: print("f ",end="") # first player hits
            else:
                print("- ",end="")
        print("|")
    print("")

def print_all_battlefield(ships, damages, visivle):
    '''
    ...
    for y in range(n):
        ...
        for x in range(n):
            pos = damages[y][x]
            if pos == -1: print("* ",end="")    # non-profit hit
            elif pos == 1: print("@ ",end="")   # good hit
