...
class nnet():
    ...
    def cnn_net(self, input_layer):
        ...
        conv2d_1 = Conv2D(self.num_channels, 3,
                          activation='linear', padding='same')(x)
        bnorm_1 = BatchNormalization(axis=3)(conv2d_1)
        a_conv1 = Activation('relu')(bnorm_1)

        conv2d_2 = Conv2D(self.num_channels, 3,
                          activation='linear', padding='same')(a_conv1)
        bnorm_2 = BatchNormalization(axis=3)(conv2d_2)
        a_conv2 = Activation('relu')(bnorm_2)

        conv2d_3 = Conv2D(self.num_channels, 3,
                          activation='linear', padding='valid')(a_conv2)
        bnorm_3 = BatchNormalization(axis=3)(conv2d_3)
        a_conv3 = Activation('relu')(bnorm_3)

        conv2d_4 = Conv2D(self.num_channels, 3,
                          activation='linear', padding='valid')(a_conv3)
    ...
    def convolutional_block(self, input_layer):
        ...
        '''
        x = Reshape((self.w, self.h, 1))(self.input_layer)
        conv2d = Conv2D(self.num_channels, 3,
                        activation='linear', padding='same')(x)
    ...
    def residual_block(self, x):
        ...
        convl_1 = Conv2D(self.num_channels, 3,
                         activation='linear', padding='same')(x)
        bnorm_2 = BatchNormalization(axis=3)(convl_1)
        activ_3 = Activation('relu')(bnorm_2)

        convl_4 = Conv2D(self.num_channels, 3,
                         activation='linear', padding='same')(activ_3)
